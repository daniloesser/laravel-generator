...
use CliGenerator\Meta\SchemaManager;
...
class Factory
{
    ...
    /**
     * @var \CliGenerator\Meta\SchemaManager
     */
    protected $schemas = [];
    ...
    /**
     * @var \CliGenerator\Support\Classify
     */
    protected $class;

    /**
     * @var \CliGenerator\Code\Model\Config
     */
    protected $config;

    /**
     * @var \CliGenerator\Code\Model\ModelManager
     */
    protected $models;

    /**
     * @var \CliGenerator\Code\Model\Mutator[]
     */
    protected $mutators = [];
    ...
    /**
     * @return \CliGenerator\Code\Model\Mutator
     */
    ...
    /**
     * @return \CliGenerator\Code\Model\ModelManager
     */
    ...
    public function create($schema, $table,$folder)
    {
        ...
        $file = $this->fillTemplate($template, $model, $folder);

        if ($model->indentWithSpace()) {
            $file = str_replace("\t", str_repeat(' ', $model->indentWithSpace()), $file);
        }

        $this->files->put($this->modelPath($model, $folder ? [$folder] : []), $file);
     ...
     * @param string $schema
     *
     * @return \CliGenerator\Meta\Schema
     */
    ...
    protected function modelPath(Model $model, $custom = [])
    {
        ...
        return $this->path([$modelsDirectory, $model->getClassName().$model->getModelPrefix().'.php']);
    }
    ...
    protected function createUserFile(Model $model)
    {
        ...
        $template = str_replace('{{class}}', $model->getClassName(), $template);
        $template = str_replace('{{ModelPrefix}}', $model->withModelPrefix(), $template);
    ...
    /**
     * @param \CliGenerator\Meta\Blueprint|null $blueprint
     * @param string $key
     * @param mixed $default
     *
     * @return mixed|\CliGenerator\Code\Model\Config
     */
